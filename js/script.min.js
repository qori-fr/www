(function($){'use strict';$(window).on('load',function(){$('.preloader').fadeOut(100);});$(window).scroll(function(){if($('.navigation').offset().top>100){$('.navigation').addClass('nav-bg');}else{$('.navigation').removeClass('nav-bg');}});const $dropdown=$(".dropdown");const $dropdownToggle=$(".dropdown-toggle");const $dropdownMenu=$(".dropdown-menu");const showClass="show";$(window).on("load resize",function(){if(this.matchMedia("(max-width: 991px)").matches){$dropdown.off("mouseenter mouseleave");}else{$dropdown.hover(function(){const $this=$(this);$this.addClass(showClass);$this.find($dropdownToggle).attr("aria-expanded","true");$this.find($dropdownMenu).addClass(showClass);},function(){const $this=$(this);$this.removeClass(showClass);$this.find($dropdownToggle).attr("aria-expanded","false");$this.find($dropdownMenu).removeClass(showClass);});}});$('.navbar-collapse').find('.dropdown').each(function(){$(this).children('a').each(function(){var url=$(this).attr('href');if(url!='#'){$(this).attr('data-url',url);$(this).attr('href','#');}});$(this).children('a').click(function(e){e.preventDefault();});if($(this).closest('.navbar-toggle').css('display')!=='none'){$(this).on({"click":function(e){if(e.target==$(this).children('a').first().get(0)){if($(this).hasClass('open')){var url=$(this).children('a').first().attr("data-url");document.location.href=url}else{$(this).addClass('open');}
